...
export default function TeamWorkspace() {
  ...
  // Real-time pending invitations for this user's email. Errors (e.g. missing
  // index, blocked by rules) are still logged to the console by the
  // subscription itself — we just don't surface them in the UI.
  useEffect(() => {
    if (!user?.email) return;
    const unsub = subscribePendingInvitations(user.email, invs => setPendingInvites(invs));
    return () => unsub();
  }, [user?.email]);
